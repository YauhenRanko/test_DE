...
from config import PLAID_CLIENT_ID_TEST, PLAID_SECRET_TEST, PLAID_PUBLIC_KEY_TEST, SCOPE, JSON_KEY_GOOGLE_FILE, TABLE_NAME
...
PLAID_CLIENT_ID = os.getenv('PLAID_CLIENT_ID' , PLAID_CLIENT_ID_TEST)
PLAID_SECRET = os.getenv('PLAID_SECRET' , PLAID_SECRET_TEST)
PLAID_PUBLIC_KEY = os.getenv('PLAID_PUBLIC_KEY', PLAID_PUBLIC_KEY_TEST)
...
PLAID_PRODUCTS = os.getenv('PLAID_PRODUCTS', 'income')
...
def get_transactions():
  ...
  try:
    ...
    dict_for_file = []
    for k in transactions_response['transactions']:
      dict_for_file.append([k['category_id'],k['category'][0],k['date'],k['amount']])
    scope = SCOPE
    creds = ServiceAccountCredentials.from_json_keyfile_name(JSON_KEY_GOOGLE_FILE, scope)
    client_google = gspread.authorize(creds)
    sheet = client_google.open(TABLE_NAME).sheet1

    for i in dict_for_file:
      index = 2
      sheet.insert_row(i,index)
      index =+1
  except plaid.errors.PlaidError as e:
    return jsonify(format_error(e))
